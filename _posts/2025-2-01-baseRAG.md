...
- a gene rator $$ p_{\theta}(y_i|x,z,y_{1:i-1}) $$ parameterized by $$\theta$$ that generates a current token based on a context of previous $$ i - 1 $$, the original input $$ x $$, and the retrieved text passages $$ z $$
